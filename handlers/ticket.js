...
module.exports = (client, preindex) => {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (!index) {
      if(!interaction.replied) await interaction.reply("Could not find the Database for your Ticket!").catch(console.warn);
      else await interaction.editReply("Could not find the Database for your Ticket!").catch(console.warn);
      return 
    }
      ...
      try {
        ...
        if (data) {
          if (data.state != "closed") {
            if(!interaction.replied) await interaction.reply({ content: `<a:no:859388931894935613> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            else await interaction.editReply({ content: `<a:no:859388931894935613> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            return
          }
        }
      } catch {
