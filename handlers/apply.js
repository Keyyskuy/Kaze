...
module.exports = async (client) => {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls, preindex = false }) {
    ...
    if(!index) {
      if(!interaction?.replied) return interaction?.reply({ephemeral: true, content: `Could not find the Database for your Application!`}).catch(() => null);
      else return interaction?.editReply({ephemeral: true, content: `Could not find the Database for your Application!`}).catch(() => null);
    }
    ...
    try {
      //COOLDOWN SYSTEM
      if (cooldown.has(user.id) && user.id != "682939235828760611") {
      ...
      else if(user.id != "682939235828760611") {
        let e = {
