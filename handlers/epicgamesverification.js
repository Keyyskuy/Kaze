    ...
    client.on("interactionCreate", async interaction => {
        ...
        if(guilddata.verifychannel == interaction.channelId && interaction.customId == "epicgamesverify" && data.epic && data.epic.length > 5) {
            ...
            user.send({
                ...
                if(others && others.length > 0) return user.send(`**Someone with the User-ID: \`${others.user}\` Linked their Account with this Epic Games Name!**`) 
                ...
                if(!tdata || tdata.code === 404) {
                    return user.send("Could not find your Epic Games Account, please try again and make sure you send the right name!")
                }
