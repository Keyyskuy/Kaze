  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      .setAuthor(client.getAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
        dynamic: true
      }), "https://discord.com/api/oauth2/authorize?client_id=965463402816348230&permissions=8&scope=bot%20applications.commands"))
      ...
      embed.addField("**<:row:965645261739262012> Rules Channel:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: "<a:no:859388931894935613> \`No Channel\`"}`, true)
      embed.addField("**<:row:965645261739262012> Public Updates Channel:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: "<a:no:859388931894935613> \`No Channel\`"}`, true)
      embed.addField("**<:row:965645261739262012> AFK Channel:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: "<a:no:859388931894935613> \`No Channel\`"}`, true)

      embed.addField("**<:row:965645261739262012> NSFW Level:**", `\`${message.guild.nsfwLevel}\``, true)
      embed.addField("**<:row:965645261739262012> Verifcation Level:**", `\`${message.guild.verificationLevel}\``, true)
      embed.addField("**<:row:965645261739262012> Explicit Content Filter:**", `\`${message.guild.explicitContentFilter}\``, true)
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "<a:boost:860434742871851048> \`" + message.guild.premiumSubscriptionCount + "\`", true)

      embed.addField(client.la[ls].cmds.info.serverinfo.field13, `<a:boost:860434742871851048> \`${boostlevel}\``, true)
      embed.addField(client.la[ls].cmds.info.serverinfo.field14, "👾 \`" + maxbitrate + " kbps\`", true)
      if(boosts >= 14){
          embed.addField(`**<:row:965645261739262012> Vanity:**`, `${message.guild.vanityURLCode ? `https://discord.gg/${message.guild.vanityURLCode}` : "<a:no:859388931894935613> No Vanity-Invite"}`)
      }
      ...
      if(message.guild.banner) {
        ...
        .setTitle(`**<:row:965645261739262012> SERVER BANNER:**`)
        ...
        let embed2 = new Discord.MessageEmbed()
        .setTitle(`**<:row:965645261739262012> SERVER DISCOVERY SPLASH:**`)
