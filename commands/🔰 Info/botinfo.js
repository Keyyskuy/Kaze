...
module.exports = {
    ...
    try{
        ...
        let tempmsg = await message.reply({embeds: [new Discord.MessageEmbed().setColor(es.color)
          .setAuthor(client.getAuthor(client.la[ls].cmds.info.botinfo.loading, "https://cdn.discordapp.com/emojis/756773010123522058.gif", "https://dsc.gg/devraiden"))
        ]}).catch(console.error)
        ...
        return Promise.all(promises)
          .then(([cluster, shards, guilds, members, ram, rssRam, ping, dbPing, players, uptime]) => {
              ...
              for(let index = 0; index<shards.length; index++) {
                ...
                .addField(
                  `${uptime[index] < 1000 || members[index] < 1 || guilds[index] < 1 ? `<:error:862306766338523166>` : `<:row:965645261739262012>`} Cluster #${cluster[index]}${cluster[index] == client.cluster.id ? ` | **Cluster of __this Guild__**` : ``}`, 
