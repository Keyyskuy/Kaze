    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Menu Apply Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://dsc.gg/devraiden'))
          .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
          
        //send the menu msg
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [row1, row2, row3, row4, new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://instagram.com/keyyskuy").setLabel("Instagram"))]
        })
        ...
        collector.on('collect', async menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `<a:no:859388931894935613> You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<a:yes:859388948026621982> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
      async function second_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor('Menu Apply Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://dsc.gg/devraiden')
          .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))
          
        //send the menu msg
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [new MessageActionRow().addComponents(Selection), new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://instagram.com/keyyskuy").setLabel("Instagram"))]
        })
        //Create the collector
        const collector = menumsg.createMessageComponentCollector({
          filter: i => i?.isSelectMenu() && i?.message.author?.id == client.user.id && i?.user,
        ...
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<a:no:859388931894935613> **You need to add at least 1 Open-Apply-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && collected2.first().mentions.channels.size > 0) {
                ...
                return message.reply("<a:no:859388931894935613> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<a:no:859388931894935613> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
          break;
          case "Add Apply Option": {
            let data = await theDB.get(`${message.guild.id}.${pre}.data`);
            if (data.length >= 25) {
              return message.reply("<a:no:859388931894935613> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<a:no:859388931894935613> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substring(0, 25);
              let index = data.findIndex(v => v.value == value);
              if(index >= 0) {
                  return message.reply("<a:no:859388931894935613> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
              ...
              let MenuEmbed = new Discord.MessageEmbed()
                .setColor(es.color)
                .setAuthor(client.getAuthor('Menu Apply Setup', 'https://cdn.discordapp.com/emojis/877653386747605032.png?size=96', 'https://dsc.gg/devraiden'))
              ...
              collector.on('collect', async menu => {
                if (menu?.user.id === cmduser.id) {
                  ...
                  if(index >= 0) {
                      return message.reply("<a:no:859388931894935613> **Options can't start the Same Apply System!** There is already an Option with that Application System!");
                  }
                  ...
                  if(!apply_for_here.has(message.guild.id) || !apply_for_here.has(message.guild.id, applypre) || !apply_for_here.has(message.guild.id, applypre+".QUESTIONS") || apply_for_here.get(message.guild.id, applypre+".QUESTIONS").length < 1) 
                    return message.reply(`<a:no:859388931894935613> **The ${applySystemExecution}. Apply System is not setupped / has no Questions, create it first with: \`${prefix}setup-apply\`**`)
          ...
          let data = await theDB.get(`${message.guild.id}.${pre}.data`);
          if (!data || data.length < 1) {
            return message.reply("<a:no:859388931894935613> **There are no Open-Apply-Options to remove**")
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          collector.on('end', collected => {
            menumsg.edit({
              embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
              components: [],
              content: `<a:yes:859388948026621982> **Selected: \`${collected.first().values[0]}\`**`
            })
          });
        }
        break;
