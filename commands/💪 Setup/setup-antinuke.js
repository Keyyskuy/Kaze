...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ;
    //only allow the Server owner, (&Keyy) to execute this Command, (Keyy just because if he needs to help for Shop Bots)
    if(message.author?.id != message.guild.ownerId){
      if(message.author?.id != "682939235828760611")
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor('Anti Nuke Setup', 'https://cdn.discordapp.com/attachments/820695790170275871/869657327941324860/PS7lwz7HwAAAABJRU5ErkJggg.png', 'https://dsc.gg/devraiden')
        ...
        async function menuselection(menu) {
          ...
          if(menu?.values[0] == "Information"){
            ...
            let MenuEmbed = new Discord.MessageEmbed()
            .setColor(es.color)
            .setAuthor('Anti Nuke Settings', 'https://cdn.discordapp.com/attachments/820695790170275871/869657327941324860/PS7lwz7HwAAAABJRU5ErkJggg.png', 'https://dsc.gg/devraiden')
            ...
            collector.on('end', collected => {
              menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yes:859388948026621982> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
            });
                ...
                if(String(index) == "0"){
                   ...
                   */
                    ...
                    async function first_layer(){
                      ...
                      let MenuEmbed = new Discord.MessageEmbed()
                        .setColor(es.color)
                        .setAuthor('Antinuke Setup', 'https://media.discordapp.net/attachments/820695790170275871/869657327941324860/PS7lwz7HwAAAABJRU5ErkJggg.png', 'https://dsc.gg/devraiden')
                      ...
                      client.on('interactionCreate', async (menu) => {
                        if (menu?.message.id === menumsg.id) {
                          if (menu?.user.id === cmduser.id) {
                            if(used33) return menu?.reply({content: `<a:no:859388931894935613> You already selected something, this Selection is now disabled!`, ephemeral: true})
                    ...
                    async function handle_the_picks_X(menuoptionindex, menuoptiondata) {
                      ...
                      await tempmsg.channel.awaitMessages({filter: m => m.author.id === cmduser.id,
                        ...
                        .then(async collected => {
                          ...
                          if(isNaN(thenumber)){
                            return message.reply(`**Your Input is not a real Number**\n> \`${String(thenumber).substring(0, 50)}\``)
                          }
            ...
            return menu?.reply({content: "<a:yes:859388948026621982> **Now using the Suggested Settings!**", embeds: [new MessageEmbed()
              .setColor(es.color)
              .setFooter(client.getFooter(es))
              .setTitle(`${await client.Anti_Nuke_System.get(message.guild.id+ ".all.enabled") ? "<a:yes:859388948026621982> __**The Anti Nuke is enabled!**__": "<a:no:859388931894935613> __**The Anti Nuke is disabled!**__"}`)
              .addField(`__**Anti Add Bot | ${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.enabled") ? "<a:yes:859388948026621982> Enabled ": "<a:no:859388931894935613> Disabled"}**__`, `>>> [Remove Roles] **Actions / Day: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.removeroles.neededdaycount")}\`**\n[Remove Roles] **Actions / Week: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.removeroles.neededweekcount")}\`\n**[Remove Roles] **Actions / Month: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.removeroles.neededmonthcount")}\`**\n\n[Kick] **Actions / Day: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.kick.neededdaycount")}\`**\n[Kick] **Actions / Week: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.kick.neededweekcount")}\`\n**[Kick] **Actions / Month: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.kick.neededmonthcount")}\`**\n\n[Ban] **Actions / Day: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.ban.neededdaycount")}\`**\n[Ban] **Actions / Week: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.ban.neededweekcount")}\`\n**[Ban] **Actions / Month: \`${await client.Anti_Nuke_System.get(message.guild.id+ ".antibot.punishment.member.ban.neededmonthcount")}\`**`)
