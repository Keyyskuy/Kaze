  ...
  run: async (client, message, args, cmduser, text, prefix, player, Tes, Tls, GuildSettings) => {
    ...
    try {
        ...
        collector.on('collect', async button => {
                ...
                .then(async collected => {
                  ...
                  try {
                    ...
                    try {
                      ...
                      return message.reply({embeds: [new MessageEmbed().setColor("RED").setTitle("INVALID COLOR ADDED").setDescription(`\`\`\`${String(e.message ? e.message : e).substring(0, 2000)}\`\`\``)]})
                    }
              ...
              await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author?.id,
                ...
                .then(async collected => {
                  ...
                  try {
                    ...
                    es = await client.settings.get(message.guild.id+".embed")
                    return message.reply({embeds: [new Discord.MessageEmbed()
                      .setTitle(`<a:yes:859388948026621982> The new Embed Footer Text is:`.substring(0, 256))
                      .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          else button?.reply({content: `<a:no:859388931894935613> You are not allowed to do that! Only: <@${cmduser.id}>`, ephemeral: true});
        });
        //Once the Collections ended edit the menu message
        collector.on('end', collected => {
          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:yes:859388948026621982> **Selected the \`${collected.first().customId}\`. Button**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
