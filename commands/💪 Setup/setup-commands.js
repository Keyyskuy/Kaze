  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    try {
      ...
      collector.on("collect", async b => {
        ...
        b?.reply(`<a:yes:859388948026621982> **\`Enabled ${enabled} Command-Categories\` and \`Disabled ${disabled} Command-Categories\` out of \`${b?.values.length} selected Command-Categories\`**`)
      })
      collector.on('end', collected => {
        msg.edit({content: "Time ran out/Input finished! Cancelled", embeds: [
          msg.embeds[0]
