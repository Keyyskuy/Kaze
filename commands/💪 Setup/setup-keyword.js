    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getFooter('Keyword Setup', 'https://images-ext-1.discordapp.net/external/HF-XNy3iUP4D95zv2fuTUy1csYWuNa5IZj2HSCSkvhs/https/emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/google/298/flexed-biceps_1f4aa.png', 'https://dsc.gg/devraiden'))
      ...
      async function handle_the_picks(optionhandletype, menuoptiondata) {
        switch (optionhandletype) { // return message.reply
          ...
          case "Delete Keyword": {
            ...
            if (!cuc || cuc.length < 1) return message.reply("There are no Custom Commands")
            let menuoptions = [
            ]
            ...
            let MenuEmbed = new Discord.MessageEmbed()
              .setColor(es.color)
              .setAuthor('Custom Command Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/google/298/flexed-biceps_1f4aa.png', 'https://dsc.gg/devraiden')
            ...
            //Once the Collections ended edit the menu message
            collector.on('end', collected => {
              menumsg.edit({ embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yes:859388948026621982> **Selected: \`${collected.first().values.length} Commands\`**` : "❌ **NOTHING SELECTED - CANCELLED**"}` })
            });
          } break;
          case "Show Settings": {
            let cuc = await client.keyword.get(message.guild.id+".commands");
            ...
            for (let i = 0; i < cuc.length; i++) {
              try {
                ...
                if (i > 13) {
                  sendembed2 = true;
                  embed2.addField(`<:row:965645261739262012> \`${cuc[i].name}\` | ${cuc[i].embed ? "Embed" : "❌ Embed"}`, ">>> " + string)
                } else
                  embed.addField(`<:row:965645261739262012> \`${cuc[i].name}\` | ${cuc[i].embed ? "Embed" : "❌ Embed"}`, ">>> " + string)
              } catch (e) {
                console.error(e)
              }
            }
