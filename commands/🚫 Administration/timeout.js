...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      if(kickmember.communicationDisabledUntilTimestamp) return message.reply("**This User is already timeouted!**");
      let time = 0;
      if(!args[1]) return message.reply(`**No time added!**\nTry something like this:\n> \`${prefix}timeout ${kickmember.id} 1h+15min Stop spamming!\``)
      let timeargs = [args[1]];
      ...
      if(!time || isNaN(time)) return message.reply(`**You added a invalid time!**\nTry something like this:\n> \`${prefix}timeout ${kickmember.id} 1h+15min Stop spamming!\``)
