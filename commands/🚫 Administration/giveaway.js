...
> This will send the winner(s) of the Giveaway, received from the Database`)
        ...
        if (args[0].toLowerCase() === "start") {
            ...
            client.giveawaysManager.reroll(giveaway.messageId, { winnerCount: !isNaN(args[1]) ? Number(args[1]) : 1})
                ...
                .catch((e) => {
                    if (e.startsWith(`Giveaway with message Id ${giveaway.messageId} is not ended.`)) {
                        ...
                        console.error(e);
                        message.reply({content : '<a:no:859388931894935613> **An error occured...**```' + String(e.message).substring(0, 1900) + "```"});
                    }
                });
            ...
            args.shift();
            if (!args[0]) {
                return message.reply({content : `The right usage of this Command is: \`${prefix}giveaway winner <GiveawayId>\` ... note that GiveawayId is the MessageId of the (Embed) Giveaway-Message`});
            }
