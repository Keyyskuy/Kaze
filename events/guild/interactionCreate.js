...
module.exports = async (client, interaction) => {
      ...
      if(!guild) {
        return interaction?.reply({content: "Interactions only Works inside of GUILDS!", ephemeral: true}).catch(()=>{});
      }
      ...
      if(client.checking[interaction.guild.id]) {
        if(command) {
          return interaction.reply("I'm setting myself up for this Guild!")
        }
        return
      }
