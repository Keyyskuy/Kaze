 ...
 //   message.author.id == "682939235828760611" ? console.log(`[${Math.floor(Date.now() - dbEvent)}ms] Received Message`.brightRed) : null
    ...
    if(client.checking[message.guild.id]) {
      if(message.content.includes(client.user.id) || message.content.startsWith(config.prefix)) {
        return message.reply("I'm setting myself up for this Guild!")
      }
      return
    }
  ...
  //  message.author.id == "682939235828760611" ? console.log(`[${Math.floor(Date.now() - dbEvent)}ms] Settings received`.brightRed) : null
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.USE_EXTERNAL_EMOJIS))
      return message.reply(`**I am missing the Permission to USE EXTERNAL EMOJIS**`).catch(console.error)
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`<a:no:859388931894935613> **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(console.error)
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`<a:no:859388931894935613> **I am missing the Permission to ADD REACTIONS**`).catch(console.error)
  ...
  //  message.author.id == "682939235828760611" ? console.log(`${message.author.id == "682939235828760611" ? ``.brightRed : ``}`.dim) : null;
    ...
    if (command && !customcmd) {
      ...
      if(musicData && musicData.channel && musicData.channel == message.channel.id){
        return message.reply("<a:no:859388931894935613> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(console.error)
      }
